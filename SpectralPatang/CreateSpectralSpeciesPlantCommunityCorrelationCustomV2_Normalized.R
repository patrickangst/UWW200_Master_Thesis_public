...
# Paths
tiff_dir <- "data/MasterThesis/RawDataClusteringMaskedNormalized/"
png_output <- "data/MasterThesis/ss_count_plot_custom_normalized/"

excel_file <- "data/MasterThesis/11_PlotClusters/Cluster_Assignement.xlsx"
output_excel_cluster <- "data/MasterThesis/11_PlotClusters/Cluster_Raster_Stats_Cluster_Custom_Normalized.xlsx"
output_excel_habitat <- "data/MasterThesis/11_PlotClusters/Cluster_Raster_Stats_Habitat_Custom_Normalized.xlsx"
...
for (tiff_path in tiff_files) {
  ...
  tiff_name_clean <- sub("_KmeansOnRawDataMasked_Normalized$", "", tiff_name)
  ...
  if (nrow(matching_entries) > 0) {
    ...
    cluster_plot_file_path <- file.path(
      png_output,
      paste0(tiff_name_clean, '_ss_count_plot_cluster_custom_normalized.png')
    )
...
for (tiff_path in tiff_files) {
  ...
  if (nrow(matching_entries) > 0) {
    ...
    cluster_plot_file_path <- file.path(
      png_output,
      paste0(tiff_name_clean, '_ss_count_plot_habitat_custom_normalized.png')
    )
