...
# Paths
tiff_dir <- "data/MasterThesis/RawDataClustering/"
png_output <- "data/MasterThesis/ss_count_plot_rawdata/"

excel_file <- "data/MasterThesis/11_PlotClusters/Cluster_Assignement.xlsx"
output_excel_cluster <- "data/MasterThesis/11_PlotClusters/Cluster_Raster_Stats_Cluster_RawData.xlsx"
output_excel_habitat <- "data/MasterThesis/11_PlotClusters/Cluster_Raster_Stats_Habitat_RawData.xlsx"
...
for (tiff_path in tiff_files) {
  ...
  tiff_name_clean <- sub("_KmeansOnRawData$", "", tiff_name)
  ...
  if (nrow(matching_entries) > 0) {
    ...
    xy_plot <- ggplot(matching_entries, aes(x = Cluster, fill = Spectral_Species)) +
      ...
      labs(
        ...
        title = paste0(tiff_name_clean, " Spectral Species Count Raw Data")
      ) + # Add title here
      theme_minimal()
