...
# Paths
test_site_name <- 'FLXTWRZONA_SD_2_SpectralSpecies.tif'
tiff_dir <- 'data/MasterThesis/03_Spectral_Species_Patrick/'
...
tiff_path <- file.path(tiff_dir, test_site_name)
...
matching_entries <- matching_entries %>%
  ...
  mutate(
    Raster_Value = paste0("RV_",Raster_Value),
...
mosaicplot(contingency_table, main = "Cluster vs Raster Value", shade = TRUE)

# Number of unique Raster_Value levels
num_vals <- length(unique(matching_entries$Raster_Value))
# Generate pastel colors
pastel_colors <- qualitative_hcl(num_vals, palette = "Pastel 1")
xy_plot <- ggplot(matching_entries, aes(x = HTS1, fill = Raster_Value)) +
  geom_bar(position = "dodge") +
  scale_fill_manual(values = pastel_colors) +
  labs(x = "Habitat type high level", y = "Count", fill = "Raster Value",
       title = paste0(tiff_name_clean," Raster Value Count")) + # Add title here
  theme_minimal()
