...
#
# # Select relevant columns and transform to df
# ground_data_pixel_values_cleaned_manova <- ground_data_pixel_values_cleaned %>%
#   select(-c(Subzone))
#
#
#
# glimpse(ground_data_pixel_values_cleaned_manova)
#
#
# # Select only the reflectance columns
# reflectance_only <- ground_data_pixel_values_cleaned_manova[, -c(1, 2)]
#
# # Remove columns with zero variance
# non_constant_cols <- apply(reflectance_only, 2, function(x)
#   sd(x, na.rm = TRUE) > 0)
# reflectance_cleaned <- reflectance_only[, non_constant_cols]
#
# # Now run PCA
# pca_result <- prcomp(reflectance_cleaned, scale. = TRUE)
#
# # Reduce to top 2–3 components
# pca_scores <- as.data.frame(pca_result$x[, 1:3])
# pca_scores$HabitatType <- ground_data_pixel_values_cleaned_manova$`Habitat Type`
#
# # Run MANOVA
# manova_result <- manova(as.matrix(pca_scores[, 1:3]) ~ HabitatType, data = pca_scores)
...
#
#
# # --- 6. Prepare data for plotting reflectance curves ---
# # Melt the reflectance data into long format
# reflectance_long <- reflectance_df %>%
#   pivot_longer(
...
#     values_to = "Reflectance"
#   ) %>%
#   mutate(Wavelength = as.numeric(Wavelength))  # Convert to numeric band number
#
# ground_data_pixel_values_cleaned_long <- ground_data_pixel_values_cleaned_long %>%
#   filter(is.finite(Reflectance))
#
#
# ggplot(
#   ground_data_pixel_values_cleaned_long,
...
#     fun.data = mean_se,
#     geom = "ribbon",
#     aes(fill = `Habitat Type`),
#     alpha = 0.2,
#     color = NA
#   ) +
#   labs(title = "Mean Reflectance Curves by Habitat Type", x = "Band (Wavelength Index)", y = "Reflectance") +
#   theme_minimal() +
