...
reflectance_norm <- reflectance_summary %>%
  ...
  mutate(
    # 1. Calculate the L2 norm (Euclidean norm) for the entire signature
...
signature_plot_by_habitat_type_norm_png_path <- file.path('data/MasterThesis/all_testsite_signatures', 'All_testsites_signature_plot_by_habitat_type_norm.png')
...
#
# --- Aggregate statistics (mean, min, max reflectance) per plot ---
reflectance_summary_minmax <- ground_data_pixel_values_cleaned_long_modified_selection %>%
  group_by(Habitat.Type, PlotID, Wavelength) %>%
  dplyr::summarize(
    MeanReflectance = mean(Reflectance, na.rm = TRUE),
    MinReflectance  = min(Reflectance, na.rm = TRUE),
...
signature_plot_by_habitat_type_norm_minmax_png_path <- file.path('data/MasterThesis/all_testsite_signatures', 'All_testsites_signature_plot_by_habitat_type_norm_minmax.png')
