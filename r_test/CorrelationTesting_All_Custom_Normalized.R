...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_custom_normalized.png",
    "correlation_habitat_types_custom_normalized.png",
    "correlation_plant_species_custom_normalized.png"
  )
)
...
write.csv(correlation_results, file.path(output_folder_path, "correlation_summary_all_custom_normalized.csv"), row.names = FALSE)
...
lm_model_input <- metrics_data_df %>%
  select(Testsite,Unique_Plant_Cummunities,Unique_Spectral_Species_WCSS_Custom_Normalized,Unique_Habitat_Types_Short3,Unique_Plant_Species) %>%
...
model_plant_communities_lm_residuals_path <- file.path(output_folder_path,"model_plant_communities_lm_residuals_all_custom_normalized.png")
...
model_plant_communities_lm_model_summary_path <- file.path(output_folder_path,"model_plant_communities_lm_model_summary_all_custom_normalized.txt")
...
# Model Habitat Type
model_habitat_types_lm <- lm(Unique_Habitat_Types_Short3 ~ Unique_Spectral_Species_WCSS_Custom_Normalized,
                             data = lm_model_input)
# anova(model_habitat_types_lm)
model_habitat_types_lm_residuals_path <- file.path(output_folder_path,"model_habitat_types_lm_residuals_all_custom_normalized.png")
...
model_habitat_types_lm_model_summary_path <- file.path(output_folder_path,"model_habitat_types_lm_model_summary_all_custom_normalized.txt")
capture.output(summary(model_habitat_types_lm), file = model_habitat_types_lm_model_summary_path)

# ggplot(lm_model_input, aes(x = Unique_Spectral_Species_WCSS_Custom_Normalized, y = Unique_Habitat_Types_Short3)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = TRUE)
...
model_plant_species_lm_residuals_path <- file.path(output_folder_path,"model_plant_species_lm_residuals_all_custom_normalized.png")
...
model_plant_species_lm_model_summary_path <- file.path(output_folder_path,"model_plant_species_lm_model_summary_all_custom_normalized.txt")
