...
clustering_index_name <- 'Unique_Spectral_Species_WCSS_Custom_Normalized'
index_name <-sub(".*_(.*)$", "\\1", clustering_index_name)

metrics_data <- read_excel(file.path(plot_metrics_path, "Plot_Metrics_Combined.xlsx"),
                           sheet = 'part_e')
output_folder_path <- 'correlation_plots_custom_normalized'
...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_part_e_custom_normalized.png",
    "correlation_habitat_types_part_e_custom_normalized.png",
    "correlation_plant_species_part_e_custom_normalized.png"
  )
)
...
for (i in 1:nrow(cor_metrics)) {
  ...
  # Get the variables
  x_data <- metrics_data_df$Unique_Spectral_Species_WCSS_Custom_Normalized
  ...
  base_plot <- ggplot(metrics_data_df, aes(x = Unique_Spectral_Species_WCSS_Custom_Normalized, y = !!yvar_sym, color = Testsite)) +
    geom_point(size = 3) +
  ...
  # Save plot
  file_name <- gsub('Normalized_','WCSS_',file_name)
...
write.csv(correlation_results, file.path(output_folder_path, "correlation_summary_part_e_custom_normalized.csv"), row.names = FALSE)
...
lm_model_input <- metrics_data_df %>%
  select(Testsite,Unique_Plant_Cummunities,Unique_Spectral_Species_WCSS_Custom_Normalized,Unique_Habitat_Types,Unique_Plant_Species) %>%
  mutate(Testsite = as.factor(Testsite))

str(lm_model_input)

# Model Plant Communities
model_plant_communities_lm <- lm(Unique_Plant_Cummunities ~ Unique_Spectral_Species_WCSS_Custom_Normalized,
                                 data = lm_model_input)
# anova(model_plant_communities_lm)
model_plant_communities_lm_residuals_path <- file.path(output_folder_path,"model_plant_communities_lm_residuals_part_e_custom_normalized.png")
...
model_plant_communities_lm_model_summary_path <- file.path(output_folder_path,"model_plant_communities_lm_model_summary_part_e_custom_normalized.txt")
capture.output(summary(model_plant_communities_lm), file = model_plant_communities_lm_model_summary_path)

# ggplot(lm_model_input, aes(x = Unique_Spectral_Species_WCSS_Custom_Normalized, y = Unique_Plant_Cummunities)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = TRUE)


# Model Habitat Type
model_habitat_types_lm <- lm(Unique_Habitat_Types ~ Unique_Spectral_Species_WCSS_Custom_Normalized,
                             data = lm_model_input)
# anova(model_habitat_types_lm)
model_habitat_types_lm_residuals_path <- file.path(output_folder_path,"model_habitat_types_lm_residuals_part_e_custom_normalized.png")
...
model_habitat_types_lm_model_summary_path <- file.path(output_folder_path,"model_habitat_types_lm_model_summary_part_e_custom_normalized.txt")
capture.output(summary(model_habitat_types_lm), file = model_habitat_types_lm_model_summary_path)

# ggplot(lm_model_input, aes(x = Unique_Spectral_Species_WCSS_Custom_Normalized, y = Unique_Habitat_Types)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = TRUE)

# Model Plant Species
model_plant_species_lm <- lm(Unique_Plant_Species ~ Unique_Spectral_Species_WCSS_Custom_Normalized,
                             data = lm_model_input)
# anova(model_plant_species_lm)
model_plant_species_lm_residuals_path <- file.path(output_folder_path,"model_plant_species_lm_residuals_part_e_custom_normalized.png")
...
model_plant_species_lm_model_summary_path <- file.path(output_folder_path,"model_plant_species_lm_model_summary_part_e_custom_normalized.txt")
capture.output(summary(model_plant_species_lm), file = model_plant_species_lm_model_summary_path)

# ggplot(lm_model_input, aes(x = Unique_Spectral_Species_WCSS_Custom_Normalized, y = Unique_Plant_Species)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = TRUE)
